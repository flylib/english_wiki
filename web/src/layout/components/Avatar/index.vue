<template>
  <a-dropdown position="bl" :style="{ top: '52px' }">
    ...
    <template #content>
      ...
      <a-doption v-if="isSuper" @click="$router.push('/admin/entry')">
        <template #icon>
          <icon-edit />
        </template>
        <template #default>条目管理</template>
      </a-doption>
      <a-doption v-if="isSuper" @click="$router.push('/admin/category')">
        <template #icon>
          <icon-folder />
        </template>
        <template #default>分类管理</template>
      </a-doption>
...
import { useIsSuperAdmin } from '@/composables/auth';
